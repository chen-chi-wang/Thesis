...
int get_ue_active_harq_pid(const uint8_t Mod_id,const uint8_t CC_id,const uint16_t rnti, const int frame, const uint8_t subframe,uint8_t *harq_pid,uint8_t *round,const uint8_t ul_flag)
{
  ...
  if (ul_flag == 0)  {// this is a DL request
    ...
    *harq_pid = -1;//
    for (i=0; i<DLSCH_ptr->Mdlharq; i++) {//8
      if (DLSCH_ptr->harq_processes[i]!=NULL) {
	if (DLSCH_ptr->harq_processes[i]->status == ACTIVE) {
        //printf("HARQ debug UE %d process harq_pid=%d is active\n",UE_id+1,i);
	  ...
	  //printf("HARQ debug harq_pid=%d is active round=%d(??) (waiting time=%d)\n",i,DLSCH_ptr->harq_processes[i]->round,sfdiff);//this round is correct
	  ...
	  if (sfdiff>sfdiff_max) { // this is an active process that is waiting longer than the others (and longer than 7 ms)
       ...
       // printf("HARQ debug 7ms round(??)=%d\n",*round);
	  }
	}else{
        //printf("HARQ debug process harq_pid=%d is not active\n",i);
    }
    ...
    if (*harq_pid == 255 && DLSCH_ptr->head_freelist != DLSCH_ptr->tail_freelist) {
      ...
      //printf("HARQ debug old condition: *harq_pid=%d head=%d tail=%d\n",*harq_pid,DLSCH_ptr->head_freelist,DLSCH_ptr->tail_freelist);
      *harq_pid = DLSCH_ptr->harq_pid_freelist[DLSCH_ptr->head_freelist];
      *round = 0;
      //printf("HARQ debug old set *harq_pid=%d *round=%d(0)\n",*harq_pid,*round);
      LOG_D(PHY,"process %d is first free process\n", *harq_pid);
    }
    printf("RB debug HARQ debug UE %d get harq_pid=%d round=%d****************\n",UE_id+1,*harq_pid,*round);
...
//    if (phy_vars_eNB->eNB_UE_stats[i].ulsch_consecutive_errors == ULSCH_max_consecutive_errors) {
	 if (phy_vars_eNB->eNB_UE_stats[i].ulsch_consecutive_errors == 1000) {
