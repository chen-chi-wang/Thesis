...
extern int CTIME;
extern const int SIZE_SMALL;//110
extern const int SIZE_BIG;//220
...
//-----------------------------------------------------------------------------
{
  ...
  if (modeP == PDCP_TRANSMISSION_MODE_TRANSPARENT) {
	...
	if (srb_flagP) {
	 // LOG_I(PDCP,"srb_flagP == TRUE");not work
      pdcp_header_len = PDCP_CONTROL_PLANE_DATA_PDU_SN_SIZE;
      pdcp_tailer_len = PDCP_CONTROL_PLANE_DATA_PDU_MAC_I_SIZE;
    } else {
	 // LOG_I(PDCP,"srb_flagP == FLASE");not work
...
int first_timestamp;
int open_counter_1=1;
int open_counter_2=1;
int free_counter=1;
int free_counter_1=1;
int free_counter_2=1;
unsigned char byte_p_1[110];//can't SIZE_SMALL
unsigned char byte_p_2[220];
//-----------------------------------------------------------------------------
boolean_t
pdcp_data_ind(
  const protocol_ctxt_t* const ctxt_pP,
...
//-----------------------------------------------------------------------------
{
...
#endif
#ifdef PDCP_MSG_PRINT
  LOG_I(PDCP,"[MSG] PDCP UL %s PDU on rb_id %d\n", (srb_flagP)? "CONTROL" : "DATA", rb_idP);

  for (i = 0; i < sdu_buffer_sizeP; i++) {
    LOG_I(PDCP,"%02x ", ((uint8_t*)sdu_buffer_pP->data)[i]);
  }
  ...
  // %EDITED%
  printf("eNB : %u, RNIT : %x, UE_instance : %d\n", ctxt_pP->module_id, ctxt_pP->rnti, ctxt_pP->instance);
  
  printf(srb_flagP ? "\nPDCP RX CONTROL\n" : "\nPDCP RX DATA\n"); 
  for (int i = 0; i < sdu_buffer_sizeP; i++) {
    printf("%02x ", ((uint8_t*)sdu_buffer_pP->data)[i]);
	if((i%10)==9){
			printf("\n");
	}
  }
  if(srb_flagP==0){
      printf("\ndata_sizeP=%d\n",sdu_buffer_sizeP-59);//57+2 pdcp hdr
      printf("sdu_buffer_sizeP=%d\n",sdu_buffer_sizeP);
  }
  //TODO
//  if(pkt_counter_1==359&&pkt_counter_2==176){
//	puts("you may close file");
//  } 

//  if(srb_flagP==false){//DATA
//
//		  if(HIGH_FPS_UE && LOW_FPS_UE) {
//				  if(open_counter_1==1){
//						  fPtr_write_1 = fopen("/home/enbuser/unix/IOTest/output_TWO_UE.yuv","wb");
//						  open_counter_1=0;
//				  }
//				  if(free_counter_1==1){//not yet close file
//						  if((PKT_COUNTER_1>=MAX_NUM_OF_LOW_FPS*UNIT_COUNTER)&&(PKT_COUNTER_2>=MAX_NUM_OF_HIGH_FPS*UNIT_COUNTER+7)){//
//								  fclose(fPtr_write_1);
//								  free_counter_1=0;
//						  }else{//you may write file
//
//								  if(fPtr_write_1){//check first
//										  if(ctxt_pP->instance==1){
//												  strncpy(byte_p_1,&((uint8_t*)sdu_buffer_pP->data)[59],SIZE_SMALL);
//												  fwrite(byte_p_1,sizeof(byte_p_1),1,fPtr_write_1);
//										  }else if(ctxt_pP->instance==2){
//												  strncpy(byte_p_2,&((uint8_t*)sdu_buffer_pP->data)[59],SIZE_BIG);
//												  fwrite(byte_p_2,sizeof(byte_p_2),1,fPtr_write_1);
//										  }
//
//								  }
//						  }
...
//		  }else if (LOW_FPS_UE){
//				  if(open_counter_1==1){
//						  fPtr_write_1 = fopen("/home/enbuser/unix/IOTest/output_LOW_UE.yuv","wb");
//						  open_counter_1=0;
//				  }
//				  if(free_counter_1==1){
...
//		  }else if (HIGH_FPS_UE){
//				  if(open_counter_1==1){
//						  fPtr_write_1 = fopen("/home/enbuser/unix/IOTest/output_HIGH_UE.yuv","wb");
//						  open_counter_1=0;
//				  }
//				  if(free_counter_1==1){
...
//				  if(ctxt_pP->instance==1){
//						  if(open_counter_1==1){
//								  fPtr_write_1 = fopen("/home/enbuser/unix/IOTest/output_CMP_UE_LOW.yuv","wb");
//								  open_counter_1=0;
//						  }
//						  if(free_counter_1==1){
//								  if((PKT_COUNTER_1>=MAX_NUM_OF_LOW_FPS*UNIT_COUNTER)){
...
//				  }else if(ctxt_pP->instance==2){
//						  if(open_counter_2==1){
//								  fPtr_write_2 = fopen("/home/enbuser/unix/IOTest/output_CMP_UE_HIGH.yuv","wb");
//								  open_counter_2=0;
//						  }
//						  if(free_counter_2==1){//not yet close file
//								  if(PKT_COUNTER_2>=MAX_NUM_OF_HIGH_FPS*UNIT_COUNTER+7){
//										  fclose(fPtr_write_2);
//										  free_counter_2=0;
//								  }else{//you may write file
//
//										  if(fPtr_write_2){//check first
//												  strncpy(byte_p_2,&((uint8_t*)sdu_buffer_pP->data)[59],SIZE_BIG);
//												  fwrite(byte_p_2,sizeof(byte_p_2),1,fPtr_write_2);
//										  }
//								  }
//						  }
//
...
//  }//DATA
  if(srb_flagP==false){
		  if(switch_counter){
				  first_timestamp=CTIME;
				  printf("My first CTIME=%d\n",first_timestamp);
  ...
  if(HIGH_FPS_UE && LOW_FPS_UE) {
      if(srb_flagP==false){
		if(open_counter==1){
			fPtr_write = fopen("/home/enbuser/unix/IOTest/output_TWO_UE.yuv","wb");
			open_counter=0;
		}
		if(free_counter==1){//not yet close file
				if((PKT_COUNTER_1>=MAX_NUM_OF_LOW_FPS*UNIT_COUNTER)&&(PKT_COUNTER_2>=MAX_NUM_OF_HIGH_FPS*UNIT_COUNTER+7)){
						fclose(fPtr_write);
						free_counter=0;
				}else{//you may write file
						if(fPtr_write){//check first
								if(ctxt_pP->instance==1){
										strncpy(byte_p_1,&((uint8_t*)sdu_buffer_pP->data)[59],110);
										fwrite(byte_p_1,sizeof(byte_p_1),1,fPtr_write);
								}else if(ctxt_pP->instance==2){
										strncpy(byte_p_2,&((uint8_t*)sdu_buffer_pP->data)[59],220);
										fwrite(byte_p_2,sizeof(byte_p_2),1,fPtr_write);
								}
						}
				}
		}
      ...
      if(srb_flagP==false){
		if(open_counter==1){
			fPtr_write = fopen("/home/enbuser/unix/IOTest/output_LOW_UE.yuv","wb");
			open_counter=0;
		}
		if(free_counter==1){
				if((PKT_COUNTER_1>=MAX_NUM_OF_LOW_FPS*UNIT_COUNTER)){
      ...
      if(srb_flagP==false){
		...
		if(open_counter==1){
			fPtr_write = fopen("/home/enbuser/unix/IOTest/output_HIGH_UE.yuv","wb");
			open_counter=0;
		}
		if(free_counter==1){
				if((PKT_COUNTER_1>=MAX_NUM_OF_HIGH_FPS*UNIT_COUNTER)){
...
#endif
    ctime = oai_emulation.info.time_ms; // avg current simulation time in ms : we may get the exact time through OCG?//hit
    if (MBMS_flagP == 0){
        // %EDITED%
		LOG_I(PDCP,
	...
	PROTOCOL_CTXT_SET_BY_MODULE_ID(
	  ...
	  RRC_DCCH_DATA_REQ (msg_p).eNB_index);
        LOG_I(PDCP, PROTOCOL_CTXT_FMT"Received %s from %s: instance %d, rb_id %d, muiP %d, confirmP %d, mode %d\n",
              ...
              RRC_DCCH_DATA_REQ (msg_p).mode);// %EDITED%
