...
#if defined(USER_MODE) && defined(OAI_EMU)
 // printf("\n%s\n","defined(USER_MODE) && defined(OAI_EMU)");//print
    ...
    while ((otg_pkt_info = pkt_list_remove_head(&(otg_pdcp_buffer[ctxt_pP->instance]))) != NULL) {
      ...
      if (otg_pkt != NULL) {
	    ...
	    src_id);
		  ...
		  // %EDITED%
		  printf("\nPDCP Source : %d, Destination : %d\n", src_id, dst_id+1);
		  printf("PDCP TX DATA : Frame %d\n", ctxt.frame);
		  for (int i = 0; i < pkt_size; i++) {
		    printf("%02x ", otg_pkt[i]);
			if((i%10)==9){
            	printf("\n");
            }
		  }
...
//EDITED
//          LOG_I(OTG,"[eNB %d] Frame %d sending packet %d from module %d on rab id %d (src %d, dst %d/%x) pkt size %d for pdcp mode %d\n",
//                ctxt.module_id,
//                ctxt.frame,
//                pkt_cnt_enb++,
//                src_id,
