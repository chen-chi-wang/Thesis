...
int rx_counter_1=0;//EDITED
int rx_counter_2=0;//EDITED
float rx_total_time=0;
//-------------------------------------------
#include "../../LAYER2/MAC/utility.h"
extern int CQI_PASS;
//-------------------------------------------
  ...
  if (buffer_tx!=NULL) {
    ...
    if (((otg_hdr_info_rx->flag == 0xffff) ||
     ...
     // LOG_I(OTG,"MAX_RX_INFO %d %d \n",NB_eNB_INST,  NB_UE_INST);//EDITED
	  ...
	  // %EDITED%
	  printf("\nOTG RX Source : %d, Destination : %d\n", otg_hdr_rx->src_instance, otg_hdr_rx->dst_instance);
...
//#warning "LG: TODO think about multicast traffic"
      ...
      if (otg_hdr_rx->time<=ctime) {
        ...
        rx_total_time+=otg_info->radio_access_delay[src_instance][dst_instance];
      ...
      if (otg_hdr_info_rx->flag == 0x1000) {
        ...
        LOG_I(OTG,"[SRC %d][DST %d] Stats :: radio access latency %.2f (tx time %d, ctime %d) jitter %.2f, CQI_PASS %d, Estimated E2E OWD:%.2f (ms):\n",
              src_instance,
              dst_instance,
