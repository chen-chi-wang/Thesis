...
#include "../../../openair2/LAYER2/MAC/utility.h"//EDITED
...
#define PRINT_STATS
  ...
  for (frame = 0;
    ...
    if (ue_connection_test == 1) {//snrnothit
      if ((frame % 20) == 0) {
        snr_dB += snr_direction;
        sinr_dB -= snr_direction;
      }

      if (snr_dB == -20) {//snrnothit
        snr_direction = snr_step;
      } else if (snr_dB == 20) {//snrnothit
        snr_direction = -snr_step;
      }
    }
...
#ifdef OPENAIR2
                        //Appligation: traffic gen
            update_otg_eNB (eNB_inst, oai_emulation.info.time_ms);//hit
			CTIME=oai_emulation.info.time_ms;
...
main (int argc, char **argv)
{
  ...
  if (create_tasks(oai_emulation.info.nb_enb_local, oai_emulation.info.nb_ue_local) >= 0) {printf("\ncreate_tasks hit\n");
    itti_wait_tasks_end();
  } else {
